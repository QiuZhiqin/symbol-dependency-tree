...
- 首次查询自动建立数据库；后续启动复用落盘数据，只重新解析时间或大小发生变化的文件。
...
2. 首次使用可运行 **Rebuild C/C++ Call Index**；不运行时，第一次查询也会自动建库。
3. 把光标放在目标函数名上，从编辑器右键菜单选择 **Generate Symbol Reference Tree**。
4. 在底部 **Symbol References** 面板的目标符号标签中使用 `+` 逐层展开调用者。
